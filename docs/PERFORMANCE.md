...
**#5 (`POST /api/downloads/batch` create)** já entregue — `DownloadModal` e `AddTorrentModal` usam `downloadBatchCreate`. Próximo alvo: **#6** (art resolve batch / `artStatus` na lista).
