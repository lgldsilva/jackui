...
func TestLocalPromoteBatch(t *testing.T) {
	...
	router := gin.New()
	router.POST("/api/local/promote", LocalPromote(b, nil, nil, shared, nil, nil, nil))
...
func TestSSRFGuards(t *testing.T) {
	...
	if err := validateFetchScheme("http://192.168.0.100/x.torrent"); err != nil {
	...
	allowed := []string{"192.168.0.100", "10.0.0.5", "172.16.0.9", "8.8.8.8"}
