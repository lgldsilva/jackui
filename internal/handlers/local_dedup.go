...
func LocalDuplicates(b *local.Browser) gin.HandlerFunc {
...
func findDuplicates(ctx context.Context, b *local.Browser, mount, scopedBase string) ([]dupGroup, error) {
...
func LocalDuplicatesDelete(b *local.Browser, dls *downloads.Store, s *streamer.Streamer) gin.HandlerFunc {
	return func(c *gin.Context) {
...
func deleteDuplicates(b *local.Browser, dls *downloads.Store, s *streamer.Streamer, mount, baseAbs string, paths []string) (int, []string) {
	deleted := 0
	errs := []string{}
	for _, p := range paths {
