...
func TestStreamUpdateSettings_SeedTrackers(t *testing.T) {
	cfg, path := newTestConfig(t)
	s := streamer.NewForTesting()
	w := putSettings(t, cfg, path, s, `{"storageBackend":"file","seedTrackers":["jackui","  ","outro"]}`)
	if w.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200; body %s", w.Code, w.Body.String())
	}
	var resp map[string]any
	...
	if len(cfg.Stream.SeedTrackers) != 2 || cfg.Stream.SeedTrackers[0] != "jackui" || cfg.Stream.SeedTrackers[1] != "outro" {
