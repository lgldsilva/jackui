...
func addDownloadCandidates(idx map[int64][]catalogCand, dls *downloads.Store, userID int, sizes map[int64]bool) {
	...
	for size := range sizes {
		...
		for _, d := range rows {
			if d.FilePath != "" {
				idx[size] = append(idx[size], catalogCand{source: "download", absPath: d.FilePath, size: size, remote: lh.DetectRemoteFS(d.FilePath)})
			}
		}
	}
}
...
func addOneMountCandidates(idx map[int64][]catalogCand, b *local.Browser, username string, m local.Mount, sizes map[int64]bool) {
	...
	remote := lh.DetectRemoteFS(m.Path)
	src := "library"
	if remote {
		src = "cloud"
	}
...
func pickMatch(f streamer.FileInfo, cands []catalogCand, pc contentid.PieceCheck, isV1 bool, torrentFP func() string) *dedupMatch {
	...
	for _, cand := range cands {
		...
		// Remote/v2 candidate → fingerprint comparison (probable). Resolve the
		// torrent fingerprint lazily, once, and only until the first probable hit.
		if !fpDone {
			fp = torrentFP()
			fpDone = true
		}
		if probable == nil {
			probable = probableMatch(f, cand, fp)
		}
	}
	return probable
}

// probableMatch returns a "probable" match when cand's fingerprint equals the
// torrent file's fingerprint fp (empty fp / read error → no match). Extracted to
// keep pickMatch's loop flat.
func probableMatch(f streamer.FileInfo, cand catalogCand, fp string) *dedupMatch {
	if fp == "" {
		return nil
	}
	if candFP, err := contentid.Fingerprint(cand.absPath, cand.size); err == nil && candFP == fp {
		return makeMatch(f, cand, "probable")
	}
	return nil
}
...
func DedupLink(dls *downloads.Store, b *local.Browser) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, it := range req.Items {
			if err := linkOneItem(dls, b, username, userID, req, it); err != nil {
...
// linkOneItem resolves+validates one link item and records the linked row. The
// per-item core of DedupLink, extracted so the handler's loop stays flat.
func linkOneItem(dls *downloads.Store, b *local.Browser, username string, userID int, req dedupLinkReq, it dedupLinkItem) error {
	...
	if statErr != nil || st.IsDir() {
		return errors.New(it.RelPath + ": not a file")
	}
	_, err = dls.CreateLinked(downloads.Download{
...
// userFromCtx returns the authenticated username, or "" when unauthenticated.
// (In package handlers; the local subpackage has its own copy — the god-file
// split moved the original out of this package. #23 dedup needs it here.)
