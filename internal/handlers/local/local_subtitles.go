...
func LocalProbe(b *lb.Browser) gin.HandlerFunc {
...
// basename overlap).
func LocalSidecars(b *lb.Browser) gin.HandlerFunc {
	return func(c *gin.Context) {
		mount := c.Query("mount")
		path := c.Query("path")
		if mount == "" || path == "" {
...
func LocalSidecarRead(b *lb.Browser) gin.HandlerFunc {
	return func(c *gin.Context) {
		mount := c.Query("mount")
		path := c.Query("path")
		name := c.Query("name")
...
func LocalSubtitleExtract(b *lb.Browser, cache *localcache.Cache) gin.HandlerFunc {
	return func(c *gin.Context) {
