...
func TestSegURLBuilder(t *testing.T) {
	builder := segURLBuilder(localSegURLOpts{mount: "Mount", path: "video.mp4", token: "mytoken", revealHidden: true})
	...
	if !strings.Contains(got, "revealHidden=1") {
		t.Errorf("expected revealHidden param, got %q", got)
	}
}

func TestSegURLBuilder_NoToken(t *testing.T) {
	builder := segURLBuilder(localSegURLOpts{mount: "Mount", path: "video.mp4"})
	got := builder("seg_00000.ts")
	if strings.Contains(got, "token=") {
		t.Errorf("expected no token param, got %q", got)
	}
	if strings.Contains(got, "revealHidden=") {
		t.Errorf("expected no revealHidden param, got %q", got)
	}
}
...
func TestListHandleError_Traversal(t *testing.T) {
	...
	listHandleError(lb.NewBrowser(nil), c, fmt.Errorf("path traversal rejected"))
...
func TestListHandleError_Other(t *testing.T) {
	...
	listHandleError(lb.NewBrowser(nil), c, fmt.Errorf("permission denied"))
...
func TestListHandleError_NotFound(t *testing.T) {
	...
	listHandleError(lb.NewBrowser([]config.ExternalMount{{Name: "Real", Path: t.TempDir()}}), c, os.ErrNotExist)
