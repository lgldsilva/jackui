...
func TestScopedOverrides_SubpathMountPrefixesUser(t *testing.T) {
	b := lb.NewBrowser([]config.ExternalMount{{Name: "Meus downloads", Path: t.TempDir(), UserSubpath: true}})
	req := &localPromoteReq{
...
func promoteRouter(b *lb.Browser, sharedDir string) *gin.Engine {
	...
	r.POST("/api/local/promote", LocalPromote(LocalPromoteDeps{Browser: b, SharedDir: sharedDir}))
	return r
}
...
func TestLocalPromote_HonoursOverride(t *testing.T) {
	...
	b := lb.NewBrowser([]config.ExternalMount{{Name: "Meus downloads", Path: src}})
