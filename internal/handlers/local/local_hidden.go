...
// the user-facing (post StripUserScope) relative path, matching lb.Entry.Path.
...
func dropHiddenLocalEntries(entries []lb.Entry, hidden map[string]bool) []lb.Entry {
	if len(hidden) == 0 {
		return entries
	}
	out := entries[:0]
...
func LocalSetHidden(b *lb.Browser, s *streamer.Streamer) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req struct {
