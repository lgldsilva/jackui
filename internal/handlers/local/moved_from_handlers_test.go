...
func TestSegURLBuilder_NativeHLS(t *testing.T) {
	withFlag := segURLBuilder(localSegURLOpts{mount: "M", path: "v.mkv", token: "TOK", nativeHLS: true})("seg_00001.ts")
	if !strings.Contains(withFlag, "native_hls=1") {
		t.Fatalf("expected native_hls=1 in seg URL, got %q", withFlag)
	}
	without := segURLBuilder(localSegURLOpts{mount: "M", path: "v.mkv", token: "TOK"})("seg_00001.ts")
	if strings.Contains(without, "native_hls") {
		t.Fatalf("did not expect native_hls when false, got %q", without)
	}
	withPlayback := segURLBuilder(localSegURLOpts{mount: "M", path: "v.mkv", token: "TOK", playback: "viewer-a"})("seg_00001.ts")
...
func Test_hgB_BuildLocalVODPlaylist_Rewrites(t *testing.T) {
	seen := []string{}
	build := segURLBuilder(localSegURLOpts{mount: "Test", path: "movie.mkv", token: "TOK"})
