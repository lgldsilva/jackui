...
func resolveLocalAudio(b *lb.Browser, c *gin.Context) (abs string, stat os.FileInfo, ok bool) {
	mount := c.Query("mount")
	path := c.Query("path")
	if mount == "" || path == "" {
...
func LocalAudioMeta(b *lb.Browser, store *audiometa.Store) gin.HandlerFunc {
...
func LocalAudioCover(b *lb.Browser, store *audiometa.Store, webSearch *imagesearch.Chain) gin.HandlerFunc {
	return func(c *gin.Context) {
		abs, stat, ok := resolveLocalAudio(b, c)
		if !ok {
			return
		...
		if t, hit := store.Get(abs, modUnix); !(hit && !t.HasCover) {
			if cover, has, err := audiometa.ReadCover(abs); err == nil && has {
				c.Header(httpshared.CacheControl, httpshared.CachePublicYear)
				c.Header("ETag", etag)
