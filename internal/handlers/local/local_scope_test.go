...
func setUpLocalScopeMount(t *testing.T) (*lb.Browser, string) {
	t.Helper()
	mountRoot := t.TempDir()
...
func newLocalRouter(b *lb.Browser, claims *auth.Claims) *gin.Engine {
	r := gin.New()
...
func scopeGet(b *lb.Browser, claims *auth.Claims, url string) *httptest.ResponseRecorder {
