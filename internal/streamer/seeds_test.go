...
func TestMatchesSeedTracker(t *testing.T) {
	tests := []struct {
		...
		{"empty trackers", []string{"https://amigos-share.club/announce/pk"}, nil, false},
		{"empty announces", nil, []string{"amigos-share"}, false},
		{"substring match", []string{"https://AMIGOS-share.club/announce/pk"}, []string{"amigos-share"}, true},
		{"no match public", []string{"udp://tracker.openbittorrent.com:80/announce"}, []string{"amigos-share"}, false},
		{"one of many matches", []string{"udp://public/announce", "https://amigos-share.club/x"}, []string{"amigos-share"}, true},
...
func TestNormalizeSeedTrackers(t *testing.T) {
	...
	want := []string{"amigos-share", "other"}
	if len(got) != len(want) {
...
func TestSetSeedTrackersLive(t *testing.T) {
	...
	if len(s.seedTrackers) != 1 || s.seedTrackers[0] != "amigos-share" {
