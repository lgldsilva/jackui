        ...
        <div className={`flex-col gap-2 ${selected ? 'hidden lg:flex' : 'flex'}`}>
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden flex-1 overflow-y-auto max-h-[calc(100vh-280px)]">
            {filteredEntries.length === 0 ? (
              ...
              <button key={entry.query} onClick={() => handleSelect(entry.query)} className={`w-full flex items-start justify-between gap-2 px-4 py-3 text-sm transition-colors border-b border-gray-700/50 last:border-b-0 text-left ${selected === entry.query ? 'bg-green-500/10 border-l-2 border-l-green-500' : 'hover:bg-gray-700/50'}`}>
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0 mt-0.5">
                  ...
                  <button onClick={e => handleDeleteEntry(entry.query, e)} title="Remover do cache" className="text-gray-600 hover:text-red-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </button>
            ))}
          </div>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredResults.slice(0, browseVisible).map((result, i) => (
                      <ResultCard key={`${result.infoHash || result.link}-${i}`} result={{ ...result, cached: true }} onDownload={setDownloadTarget} onPlay={(r) => playSingle(r)} onAddToPlaylist={(r) => { setPlaylistTargetFile(null); setPlaylistTarget(r) }} onExploreContents={setContentsTarget} onRefresh={handleRefreshResult} refreshing={result.id !== undefined && refreshingIDs.has(result.id)} refreshedAt={result.id === undefined ? null : refreshedLabels.get(result.id) ?? null} />
                    ))}
                  </div>
