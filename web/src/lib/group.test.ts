...
describe('jackui regression: a hash-less listing must not be absorbed by a hash-bearing one', () => {
  // Private trackers (jackui & co.) come through Jackett as .torrent-only
  ...
  it('keeps the jackui result visible alongside a same-title/size magnet result', () => {
    const out = groupByInfoHash([
      ...
      mk({ title: 'Filme Obscuro 2024 1080p', size: 5_000_000_000, tracker: 'jackui', seeders: 2, infoHash: '', magnetUri: '', link: 'https://jackui.example/t/123.torrent' }),
    ])
    // BUG (current): out.length === 1 — jackui collapsed into the magnet card's alsoIn.
    expect(out).toHaveLength(2)
