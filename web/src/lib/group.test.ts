...
describe('amigos-share duplicate regression: same tracker with and without infoHash', () => {
  ...
  it('suppresses the noHash duplicate when same tracker already has a hash-bearing entry for same title+size', () => {
    const out = groupByInfoHash([
      mk({ title: 'Filme Privado 2024 1080p', size: 4_000_000_000, tracker: 'amigos-share', seeders: 5, infoHash: HASH2, magnetUri: `magnet:?xt=urn:btih:${HASH2}` }),
      mk({ title: 'Filme Privado 2024 1080p', size: 4_000_000_000, tracker: 'amigos-share', seeders: 5, infoHash: '', magnetUri: '', link: 'https://amigos-share.example/t/456.torrent' }),
    ])
    expect(out).toHaveLength(1)
    expect(out[0].infoHash).toBe(HASH2)
  })

  it('keeps BOTH when the noHash entry is from a DIFFERENT tracker (original amigos-share fix must hold)', () => {
    const out = groupByInfoHash([
