...
export default function AccountCard() {
  ...
  const addPasskey = async () => {
    ...
    try {
      ...
      await loadPasskeys()
      setPkMsg(t('account.passkey_added'))
    } catch (e: any) {
      if (e?.name === 'NotAllowedError' || e?.name === 'AbortError') setPkMsg(t('account.cancelled'))
      else setPkMsg(e?.response?.data?.error || e.message || t('account.passkey_add_failed'))
    } finally { setPkBusy(false) }
  }
  const removePasskey = async (id: string) => {
    setPkMsg('')
    try { await passkeyDelete(id); await loadPasskeys() }
    catch (e: any) { setPkMsg(e?.response?.data?.error || t('account.remove_failed')) }
  }
  ...
  const startEnroll = async () => {
    setMfaMsg('')
    try { setEnroll(await mfaEnroll()) } catch (e: any) { setMfaMsg(e?.response?.data?.error || t('account.mfa_start_failed')) }
  }
  const confirmEnroll = async () => {
    ...
    try {
      ...
      setMfaMsg(t('account.mfa_enabled'))
    } catch (e: any) { setMfaMsg(e?.response?.data?.error || t('account.code_invalid')) }
  }
  const disableMfa = async () => {
    setMfaMsg('')
    try { await mfaDisable(mfaPw); setMfaPw(''); setBackupCodes(null); setBackupRemaining(null); await refresh(); setMfaMsg(t('account.mfa_disabled')) }
    catch (e: any) { setMfaMsg(e?.response?.data?.error || t('account.wrong_password')) }
  }
  ...
  const submit = async () => {
    setMsg(null)
    if (next.length < 6) { setMsg({ ok: false, text: t('account.password_min') }); return }
    if (next !== confirm) { setMsg({ ok: false, text: t('account.password_mismatch') }); return }
    setBusy(true)
    try {
      ...
      setMsg({ ok: false, text: e?.response?.data?.error || t('account.password_change_failed') })
  ...
  return (
    <section className="card flex flex-col gap-3">
      <h2 className="text-lg font-semibold text-text-primary flex items-center gap-2"><User className="w-5 h-5" /> {t('account.title')}</h2>
      <p className="text-sm text-text-secondary flex items-center gap-1.5 flex-wrap">
        <span>{user.username}{displayEmail ? ` · ${displayEmail}` : ''} · {user.role === 'admin' ? t('account.role_admin') : t('account.role_user')}</span>
      ...
      <div className="flex flex-col gap-2 max-w-sm pt-3 border-t border-default/60">
        <label className="text-xs text-text-secondary flex items-center gap-1.5"><KeyRound className="w-3.5 h-3.5" /> {t('account.change_password')}</label>
        <input type="password" value={current} onChange={e => setCurrent(e.target.value)} placeholder={t('account.current_password_placeholder')} autoComplete="current-password"
          className="bg-surface border border-default rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-green-500" />
        <input type="password" value={next} onChange={e => setNext(e.target.value)} placeholder={t('account.new_password_placeholder')} autoComplete="new-password"
          className="bg-surface border border-default rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-green-500" />
        <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder={t('account.confirm_new_password_placeholder')} autoComplete="new-password"
          className="bg-surface border border-default rounded-lg px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-green-500" />
        <div className="flex items-center gap-3">
          <button onClick={submit} disabled={busy || !current || !next}
            className="flex items-center gap-1.5 text-sm bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white rounded-lg px-3 py-1.5">
            {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <KeyRound className="w-4 h-4" />} {t('account.save')}
          </button>
      ...
      <div className="flex flex-col gap-2 pt-3 border-t border-default/60 max-w-sm">
        <span className="text-xs text-text-secondary flex items-center gap-1.5">
          ...
          {t('account.mfa_label')} {user.mfaEnabled ? t('account.mfa_active') : t('account.mfa_inactive')}
        </span>

{(() => {
          if (user.mfaEnabled) return <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <input type="password" value={mfaPw} onChange={e => setMfaPw(e.target.value)} placeholder={t('account.password_to_disable')} autoComplete="current-password"
                className="bg-surface border border-default rounded-lg px-3 py-1.5 text-sm text-text-primary flex-1" />
              <button onClick={disableMfa} disabled={!mfaPw} className="text-sm bg-surface-tertiary hover:bg-surface-tertiary disabled:opacity-50 text-text-primary rounded-lg px-3 py-1.5">{t('account.disable')}</button>
            </div>
            <div className="flex items-center gap-2 flex-wrap text-xs text-text-secondary">
              <LifeBuoy className="w-3.5 h-3.5" />
              <span>{t('account.backup_codes_remaining', { count: backupRemaining ?? '—' })}</span>
              {backupRemaining !== null && backupRemaining <= 2 && (
                <span className="text-amber-400">{t('account.backup_codes_few')}</span>
              )}
              {showRegen ? (
                <span className="inline-flex items-center gap-1">
                  <input type="password" value={regenPw} onChange={e => setRegenPw(e.target.value)} placeholder={t('account.password_placeholder')} autoComplete="current-password"
                    className="bg-surface border border-default rounded px-2 py-1 text-text-primary w-28" />
                  <button onClick={regenerateBackup} disabled={!regenPw} className="text-green-400 hover:underline disabled:opacity-50">{t('account.confirm_lower')}</button>
                  <button onClick={() => { setShowRegen(false); setRegenPw('') }} className="text-text-muted hover:text-text-primary">{t('account.cancel')}</button>
                </span>
              ) : (
                <button onClick={() => setShowRegen(true)} className="text-blue-400 hover:underline inline-flex items-center gap-1">
                  <RefreshCw className="w-3 h-3" /> {t('account.generate_new')}
                </button>
              )}
            </div>
          </div>
          if (enroll) return <div className="flex flex-col gap-2">
            <p className="text-xs text-text-secondary">{t('account.mfa_enroll_hint')}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={confirmEnroll} disabled={mfaCode.length !== 6} className="text-sm bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white rounded-lg px-3 py-1.5">{t('account.confirm')}</button>
              <button onClick={() => { setEnroll(null); setMfaCode('') }} className="text-xs text-text-muted hover:text-text-primary">{t('account.cancel')}</button>
            </div>
          </div>
          return <button onClick={startEnroll} className="self-start flex items-center gap-1.5 text-sm bg-surface-tertiary hover:bg-surface-tertiary text-text-primary rounded-lg px-3 py-1.5">
            <ShieldCheck className="w-4 h-4" /> {t('account.enable_mfa')}
          </button>
        })()}
        ...
        {backupCodes && backupCodes.length > 0 && (
          <div className="flex flex-col gap-2 bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
            <p className="text-xs text-amber-700 dark:text-amber-300 flex items-center gap-1.5">
              <LifeBuoy className="w-3.5 h-3.5" /> {t('account.backup_codes_warning')}
            </p>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                {copied ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />} {t('account.copy_all')}
              </button>
              <button
                onClick={() => {
                ...
                className="text-xs text-text-secondary hover:text-text-primary">{t('account.download_txt')}</button>
              <button onClick={() => setBackupCodes(null)} className="text-xs text-text-muted hover:text-text-primary ml-auto">{t('account.saved_it')}</button>
            </div>
          </div>
        )}
      </div>
      ...
      {passkeysSupported && (
        <div className="flex flex-col gap-2 pt-3 border-t border-default/60 max-w-sm">
          <span className="text-xs text-text-secondary flex items-center gap-1.5">
            ...
            {t('account.passkeys_label')} {passkeys.length > 0 ? `— ${passkeys.length}` : t('account.passkeys_none')}
          </span>
          {passkeys.length > 0 && (
            <ul className="flex flex-col gap-1">
              {passkeys.map(pk => (
                <li key={pk.id} className="flex items-center gap-2 bg-surface border border-default rounded-lg px-2 py-1">
                  ...
                  <button onClick={() => removePasskey(pk.id)} title={t('account.remove')} className="text-text-muted hover:text-red-400 flex-shrink-0">
          ...
          <button onClick={addPasskey} disabled={pkBusy}
            ...
            {pkBusy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Fingerprint className="w-4 h-4" />} {t('account.add_passkey')}
          </button>
