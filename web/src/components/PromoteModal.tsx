...
import { DownloadEntry, downloadPromoteBrowse, downloadPromoteBatch, fetchPromoteDestinations, PromoteDestination } from '../api/client'
  ...
  const handlePromote = async () => {
    ...
    try {
      const result = await downloadPromoteBatch(
        items.map(i => i.id),
        { keepSeeding, targetSubdir: finalTarget, targetBase: selectedBase || undefined },
      )
      onPromoted(result)
    } catch (e: unknown) {
